	...
	{ // 先頭がない==プライオリティまるっとない
		...
		while (pObject != nullptr)
		{
			...
			{ // 対象のモデルのとき
				...
				case CObject::OBJECT_BLOCK3D:
					pBlock3D = (CBlock3D*)pObject;

					EscData = pBlock3D->GetDATA();
					break;
				...
				case CObject::OBJECT_MOTIONENEMY_NOMAL:
					...
					break;
				case CObject::OBJECT_MOTIONENEMY_FAST:
					pEnemyFast = (CObjectMotionEnemyfast*)pObject;

					EscData = pEnemyFast->GetClassData();
					break;
				...
				else
				{
					...
					{//半径同士合算より現在の距離が近い時==接触
						...
						case CObject::OBJECT_MOTIONENEMY_FAST:
							...
							//当たり判定計算
							EscHitData = CMathProc::CheckBoxCollision_3D(TargetType, EscData.Pos, EscData.OldPos, EscData.MinLength, EscData.MaxLength, CObject::OBJECT_HITBOX, CObject::LAYERINDEX_HITBOX, EscMove, (CObjectMotionEnemyfast*)pObject);

							EscHitData2 = CMathProc::CheckBoxCollision_3D(TargetType, EscData.Pos, EscData.OldPos, EscData.MinLength, EscData.MaxLength, CObject::OBJECT_OBSTACLE, CObject::LAYERINDEX_OBSTACLE, EscMove, (CObjectMotionEnemyfast*)pObject);
						...
						//相手データに更新あり、変動の必要性あり
							...
							{//半径同士合算より現在の距離が近い時==接触
								...
								case CObject::OBJECT_MOTIONENEMY_FAST:
									...
									pEnemyfast->SetClassData(EscData);
...
void CMathProc::LoadRouteCSV(const char* filename)
{
	...
	{
		std::istringstream lineStream(line); // 読み込んだ行を解析するためのストリーム
		std::string cell;                    // 各セルのデータを格納する変数
		int x = 0;                            // 列のインデックス

		// 行をカンマ区切りで分割し、各セルのデータを取得
		while (std::getline(lineStream, cell, ',') && x < GRIDROW * GRIDCOL)
		{
			// 取得したセルの値を整数に変換し、ルートテーブルに格納
